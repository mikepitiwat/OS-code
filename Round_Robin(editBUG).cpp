...
int func_FCFS(int index){                 // โปรเซสเกิดพร้อมกัน ใช้ FCFS
    ...
    for (int j = 0; j < n; j++) {      // นำโปรเตัวที่เหลือไปต่อคิว ยกเว้นโปรเซสที่ก่อน
        if(temp[j] != index){
...
void Round_Robin(){
    int runingP = 0, timeleft, index, END_P = 0, statusQ = 1;
        ...
        for (int j = 1; j <= NP; j++) {
            if (i == P[j].ArrivalT) {                   // ณ เวลาที่ i มี process[j] เกิดขึ้น
                ...
                if (i >= END_P && (F ==0 && R == 0 )) { // ถ้าจบQuantum time = โปรเซสสามารเข้าไปทำงานได้
                    ...
                    duplicate(index);                   // ถ้ามีโปรเซสเกิดขึ้นพร้อมกันเก็บให้เก็บโปรเซสไว้ในคิว
                    break;
                }
            }
        }
